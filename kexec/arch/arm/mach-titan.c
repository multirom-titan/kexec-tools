...
static uint32_t titan_dtb_compatible(void *dtb, struct msm_id *devid, struct msm_id *dtb_id)
{
    int root_offset;
    const void *prop;
    int len;
...
static int titan_choose_dtb(const char *dtb_img, off_t dtb_len, char **dtb_buf, off_t *dtb_length)
{
    ...
    while(dtb + sizeof(struct fdt_header) < dtb_end)
    {
        ...
        if(titan_dtb_compatible(dtb, &devid, &dtb_id))
        {
...
static int titan_add_extra_regs(void *dtb_buf)
{
    FILE *f;
    uint32_t reg;
    int res;
...
const struct arm_mach arm_mach_titan = {
    .boardnames = { "titan", NULL },
    .choose_dtb = titan_choose_dtb,
    .add_extra_regs = titan_add_extra_regs,
};
